...
{
    static int seconds = 1800; // 计时器初始值, 1800 次 100 毫秒 -> 180 秒
    ...
    case WM_PAINT: {
        ...
        // 显示计时器信息
        int displayTime = seconds / 10; // 转换为秒，100ms * 10 = 1s
        TCHAR buffer[50];
        _stprintf_s(buffer, _T("Time: %d s"), displayTime);
    ...
    case WM_TIMER: {
        ...
        if (wParam == 1) {
            if (seconds > 0) {
                ...
                MessageBox(hWnd, _T("时间到！游戏失败！"), _T("游戏结束"), MB_OK);
